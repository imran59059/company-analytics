...
4. Employee count${employeeCount ? ` (verify against reported: ${employeeCount})` : ''}
...
- For Indian companies: Check MCA, Tofler, Zaubacorp${gstinNumber ? `\n- Verify GSTIN: ${gstinNumber}` : ''}
...
async function* generateAnalysisWithCompanyDetails(companyName, companyDetails, employeeCount = null, gstinNumber = null) {
  ...
  try {
    ...
    const streamingResponse = await openAIClient.chat.completions.create({
      ...
      stream: true,
      // temperature: 0.3,
      // max_tokens: 2000, // Limit to keep response concise
    });
...
async function* generateCompanyVoiceReviews(companyName) {
  ...
  try {
    ...
    const streamingResponse = await openAIClient.chat.completions.create({
      ...
      // max_tokens: 800, // Short and focused
    });
...
expressApp.get("/api/company-analytics/:uuid", async (req, res) => {
  ...
  try {
    const [rows] = await pool.execute(
      `SELECT uuid, company_name, number_of_employees, company_gstin, model, latency_ms, 
